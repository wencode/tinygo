...
func (usbcdc *USBCDC) Flush() error {
	if usbLineInfo.lineState > 0 {
		...
		if 0 < sz {
			...
			// set the data
			usbEndpointDescriptors[usb_CDC_ENDPOINT_IN].DeviceDescBank[1].ADDR.Set(uint32(uintptr(unsafe.Pointer(&udd_ep_in_cache_buffer[usb_CDC_ENDPOINT_IN][idx&0xC0]))))
			usbcdc.TxIdx.Set(0x40 - idx&0xC0)
